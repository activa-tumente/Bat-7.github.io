...
-- -----------------------------------------------------
-- Ejemplo de consulta para obtener el Pc a partir de una PD
-- -----------------------------------------------------
-- Supongamos que queremos encontrar el Pc para el Factor 'V' con una Puntuación Directa (PD) de 26

/*
SELECT Pc
FROM Baremos_ESO_E_12_13
WHERE Factor = 'V'
  AND PD_Min IS NOT NULL -- Asegurarse de que haya un rango definido
  AND 26 >= PD_Min        -- La PD dada debe ser mayor o igual al mínimo del rango
  AND 26 <= PD_Max;       -- La PD dada debe ser menor o igual al máximo del rango

-- Otro ejemplo: Factor 'A', PD = 40
SELECT Pc
FROM Baremos_ESO_E_12_13
WHERE Factor = 'A'
  AND PD_Min IS NOT NULL
  AND 40 >= PD_Min
  AND 40 <= PD_Max;

-- Otro ejemplo: Factor 'M', PD = 10
SELECT Pc
FROM Baremos_ESO_E_12_13
WHERE Factor = 'M'
  AND PD_Min IS NOT NULL

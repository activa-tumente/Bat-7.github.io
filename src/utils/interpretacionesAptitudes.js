export const interpretacionesAptitudes = {
  V: { // Aptitud Verbal
...
export const obtenerInterpretacionAptitud = (codigo, percentil) => {
  const aptitud = interpretacionesAptitudes[codigo];
  if (!aptitud) return null;

  const esAlto = percentil >= 70; // Percentil 70 o superior se considera alto
  
  return {
    descripcion: aptitud.descripcion,
    caracteristicas: esAlto ? aptitud.altas : aptitud.bajas,
    nivel: esAlto ? 'alto' : 'bajo'
  };
};

export default interpretacionesAptitudes;
